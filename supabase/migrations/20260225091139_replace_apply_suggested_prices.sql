-- Update: Inclusão de parâmetro para filtrar preços de concorrentes zerados
-- Created at: 2026-02-23

CREATE OR REPLACE FUNCTION public.apply_suggested_prices(
    p_flyer_id INT, 
    p_only_zero_comp_price BOOLEAN DEFAULT FALSE
)
RETURNS VOID AS $$
BEGIN
    ...
    FROM (
        ...
        WHERE pfp_inner.promotional_flyer_id = p_flyer_id
          ...
          -- Se o parâmetro for TRUE, filtra apenas itens com preço do concorrente zerado
          AND (p_only_zero_comp_price = FALSE OR vars.min_comp_price <= 0)
    ) calc
